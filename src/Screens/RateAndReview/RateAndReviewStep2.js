...
export default class RateAndReviewStep2 extends Component {
    ...
    callAlert = () => {
        if (this.state.inputValue === '') {
            ...
            this.props.navigation.navigate('RateAndReviewStep3', {id: this.props.navigation.getParam('id')})
        }
    }
            ...
            <View style={styles.containerStyle}>
                <View style={styles.headerView}>
                    ...
                    <View style={styles.skipView} >
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('RateAndReviewStep3', {id: this.props.navigation.getParam('id')})} >
                            <Text style={styles.skipText} > {strings.SKIP} </Text>
                        </TouchableOpacity>
                    </View>
                </View>
                <Text style={styles.stepText} > {strings.rateStep2} </Text>
                <Progress.Bar style={styles.progressBar} progress={this.state.valid ? 67/100 : 33 / 100} width={vw(388)} color={Colors.green} unfilledColor={Colors.progressBarColor} borderColor={Colors.progressBarColor} animated={true} />
                <View style={styles.bottomView} >
                    <Text style={styles.rateHost} > {strings.joinAgain} </Text>
                    <Image source={Images.recommend} style={styles.maleImage} />
                    ...
                    <LinearGradient colors={this.state.valid ? colors : colors2} start={{ x: 1, y: 0 }} end={{ x: 0, y: 1 }} style={styles.buttonStyle}  >
                        ...
                        >
                            <VectorIcons.Ionicons name='ios-arrow-round-forward' size={vh(55)} color={this.state.valid ? Colors.white : Colors.stars} style={styles.icon} />
                        </TouchableOpacity>
                    </LinearGradient>
                </View>
